"""
Core Application class - The heart of the Velox framework

This module implements the main Velox application class that orchestrates
...
from velox.core.events import EventBus
from velox.core.exceptions import VeloxException
from velox.core.registry import AppRegistry
from velox.http.middleware import MiddlewareStack
from velox.http.routing import Router
from velox.db.connection import get_database, connect_database, disconnect_database
...
class Velox:
    """
    Main Velox framework application class.
    ...
    def __init__(
        ...
        title: str = "Velox Application",
        description: str = "Built with Velox Framework",
        version: str = "1.0.0",
        **kwargs
    ):
        """
        Initialize the Velox application.
    ...
    async def startup(self) -> None:
        ...
        logger.info("Starting Velox application...")
        ...
        try:
            ...
            self._started = True
            logger.info("Velox application started successfully")
            
        except Exception as e:
            logger.error(f"Failed to start application: {e}")
            raise VeloxException(f"Startup failed: {e}") from e
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Shutting down Velox application...")
        ...
        try:
            ...
            self._started = False
            logger.info("Velox application shut down successfully")
            
        except Exception as e:
            logger.error(f"Error during shutdown: {e}")
            raise VeloxException(f"Shutdown failed: {e}") from e
    ...
    def create_asgi_app(self) -> Starlette:
        ...
        # Convert Velox routes to Starlette routes
        routes = []
    ...
    def __repr__(self) -> str:
        return f"<Velox('{self.title}', debug={self.debug})>"
