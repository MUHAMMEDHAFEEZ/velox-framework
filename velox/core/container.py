"""
Dependency Injection Container for Velox framework
...
class Container:
    ...
    def resolve(self, name: str) -> Any:
        ...
        Raises:
            VeloxException: If service not found or circular dependency detected
        """
        # Check for circular dependencies
        if name in self._resolving:
            cycle = " -> ".join(self._resolving + [name])
            raise VeloxException(f"Circular dependency detected: {cycle}")
        ...
        if name not in self._services:
            raise VeloxException(f"Service '{name}' not registered")
    ...
    def resolve_by_type(self, service_type: Type[T]) -> T:
        ...
        raise VeloxException(f"No service registered for type '{service_type.__name__}'")
