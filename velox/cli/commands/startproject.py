"""
Command to create a new Velox project.

This module provides functionality to scaffold a new Velox project
...
def create_project(name: str, project_path: Path, template: Optional[str] = None) -> None:
    """
    Create a new Velox project with standard structure.
...
This file contains the main configuration for your Velox application.
For more information see: https://docs.veloxframework.org/configuration/
"""
...
# Middleware
MIDDLEWARE = [
    "velox.security.middleware.SecurityMiddleware",
    "velox.http.middleware.RequestLoggingMiddleware",
    "velox.auth.middleware.AuthenticationMiddleware",
    "velox.http.middleware.ErrorHandlerMiddleware",
]
...
'''
    ...
    base_requirements = """# Base requirements for Velox project
velox-framework>=0.1.0
...
This is the entry point for your Velox application.
"""
...
def create_app(settings_override=None) -> Velox:
    """
...
from velox.core.registry import AppConfig
...
class Example(BaseModel):
    """Example model to demonstrate Velox ORM."""
...
from velox.http import Router, JsonResponse
from velox.http.controllers import APIController
...
class HomeController(APIController):
    ...
    async def index(self, request):
        ...
        return self.success_response({
            "message": "Welcome to Velox Framework!",
            "version": "0.1.0",
            "docs": "/docs"
        })
...
from velox.admin import admin, ModelAdmin
...
import pytest
from velox.test import TestCase
from .models import Example
...
class HomeControllerTests(TestCase):
    ...
    async def test_hello_endpoint(self):
        """Test hello endpoint."""
        response = await self.client.get("/api/hello/Velox")
        assert response.status_code == 200
        
        data = response.json()
        assert data["message"] == "Hello, Velox!"
'''
    }
...
A modern web application built with the Velox Framework.
...
RUN velox-admin collectstatic --noinput
